    ...
    <div className='prose prose-invert prose-ul:list-disc prose-ol:list-decimal prose-li:marker:text-brand-400 max-w-none'>
      <ReactMarkdown
        ...
        components={{
          ...
          // Blockquotes
          blockquote: ({ children }) => (
            <blockquote className='border-l-4 border-brand-500/50 bg-gray-900/50 pl-4 py-2 my-4 rounded-r-lg text-gray-400 italic'>
              {children}
            </blockquote>
          ),
          ...
          ul: ({ children }) => (
            <ul style={{ listStyleType: 'disc', paddingLeft: '1.5rem' }} className='space-y-2 text-gray-300 my-6 marker:text-brand-400'>{children}</ul>
          ),
          ol: ({ children }) => (
            <ol style={{ listStyleType: 'decimal', paddingLeft: '1.5rem' }} className='space-y-2 text-gray-300 my-6 marker:text-brand-400'>{children}</ol>
          ),
          li: ({ children, ...props }) => {
